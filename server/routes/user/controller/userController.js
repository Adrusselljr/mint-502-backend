...
const createUser = async (req, res) => {
    ...
    try {
        ...
        const newCart = new Cart({
            cartOwner: savedUser._id
        })
        const savedCart = await newCart.save()

        savedUser.userCart = savedCart._id
        await savedUser.save()
        res.status(200).json({ message: "New user has been saved, and cart created for user.", payload: savedUser, newCart: savedCart })
    }
    catch (err) {
        console.log(err)
...
const updatePassword = async (req, res) => {
    ...
    try {
        ...
        const updatedUser = await User.findOneAndUpdate({ _id: decodedToken._id }, req.body, { new: true })
        if(!updatedUser) throw new Error("No user with id found")
        res.status(200).json({ message: "Updated password", payload: updatedUser })
    }
    catch (err) {
        console.log(err)
        res.status(500).json({ error: errorHandler(err) })
